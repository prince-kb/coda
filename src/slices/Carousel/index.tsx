...
const Carousel = ({ slice }: CarouselProps): JSX.Element => {
  ...
  function changeFlavor(index: number) {
    ...
    tl.to(sodaCanRef.current.rotation,{
      y:index>currentFlavorIndex?`-=${Math.PI*2*SPINS}` : `+=${Math.PI*2*SPINS}`,
      duration:1,
      ease : "power2.inOut"
    },0)
    ...
    .to({},{onStart : ()=>setCurrentFlavorIndex(nextIndex)},.5)
    .to(".text-wrapper",{
      duration:.2,
      y:0,
      opacity:1
...
function ArrowButton({label,onClick,direction="right"}:ArrowButtonProps) {
  return (
    <button className="size-12 rounded-full border-2 border-white bg-white/10 p-3 opacity-85 ring-white focus:outline-none focus-visible:opacity-100 focus-visible:ring-4 md:size-16 lg:size-20" onClick={onClick}> <ArrowIcon className={clsx(direction==="right" && '-scale-x-100')}/> <span className="sr-only">{label}</span></button>
  );
}